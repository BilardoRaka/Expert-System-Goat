...
<div class="card text-white bg-success mb-3">
    ...
    <div class="card-body">
        <p>Halo {{ $riwayat->peternak->nama }} ({{ $riwayat->peternak->umur }} tahun), kami menarik kesimpulan bahwa kambing anda menderita <b>{{ $riwayat->penyakit->penyakit }}</b></p>
        <p>Gejala yang Dialami :
        <ol>
            @foreach($symptoms as $symptom)
            <li>{{ $symptom->gejala->gejala }}</li>
            @endforeach
        </ol>
        </p>
        <p>Penanganan yang dapat anda lakukan :
            {!! $riwayat->penyakit->solusi !!}</p>
    ...
    <div class="card-body">
        ...
        <div class="col-lg-9">
            <form method="post" action="/feedback">
                @csrf
                <input type="hidden" name="nama" value="{{ $riwayat->peternak->nama }}">
                <input type="hidden" name="umur" value="{{ $riwayat->peternak->umur }}">
